...
public static class QuizEndpoints
{
    public static void MapQuizEndpoints(this WebApplication app)
    {
        ...
        // Returns our DTO
        app.MapGet("/basicquiz", async (IQuizGenerator quizGenerator, string topic, int aiService, int model,
                string language = "en", int numQuestions = 5, int numOptions = 4, int extractLength = 1000) =>
            {
                Log.Verbose(
                    $"GET /basicquiz called with topic '{topic}' in '{language}' with {numQuestions} questions, {numOptions} options, and {extractLength} extract length.");
                ...
                {
                    Languages
                        lang = LanguagesExtensions
                            .GetLanguageFromCode(language); // this will throw error if language is not found

                    var quiz = await quizGenerator.GenerateBasicQuizAsync(topic, lang, aiService, model, numQuestions,
                        numOptions, extractLength);
            ...
            .WithOpenApi(operation =>
            {
                ...
                operation.Description =
                    "This endpoint generates a quiz only on the topic entered. The user can specify the topic that matches the langauge code entered (eg: en, fr, zh, es). " +
        ...
        app.MapPost("/submitquiz", async (IQuizRepository quizRepository, SubmissionDto submissionDto) =>
            ...
            .WithOpenApi(operation =>
            {
                ...
                operation.Description =
                    "This endpoint allows users to submit their answers for a quiz and receive feedback on their performance.";
                return operation;
            });
    }
